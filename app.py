...
if not api_key:
    # 이 부분은 키가 이미 입력되어 있으므로 무시될 것입니다.
    st.error("⚠️ API Key가 설정되지 않았습니다.")
    st.stop()

# Gemini 클라이언트 초기화
client = genai.Client(api_key=api_key)
model_name = 'gemini-2.5-flash' # 빠르고 가성비 좋은 모델 선택

# -----------------------------------------------------
# 2. 핵심 프롬프트 (System Instruction) 정의
# -----------------------------------------------------
SYSTEM_INSTRUCTION = """
...
[입력 명제 검토 원칙]
1. **수학적 명제 한정**: 명제에 포함된 집합(자연수, 실수, 집합, 도형 등)이나 조건이 수학과 관련이 없는 경우, 답변 대신 "수학과 관련된 명제를 입력해주세요."라고 출력합니다.
...
st.set_page_config(page_title="고1 명제 논리 튜터 챗봇", layout="centered")
st.title("👨‍🏫 고1 명제 논리 튜터 챗봇")
st.markdown("명제를 입력하고, 본인의 판단과 이유를 적어 **즉각적인 논리 피드백**을 받아보세요.")
...
if submit_button:
    ...
    with st.spinner('✨ AI 튜터가 논리를 분석하고 피드백을 생성 중입니다...'):
        try:
            ...
            st.error(f"API 호출 중 오류가 발생했습니다. 키 설정을 확인하거나, 잠시 후 다시 시도해 주세요. (오류: {e})")
